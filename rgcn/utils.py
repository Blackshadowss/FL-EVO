...
def filter_score(test_triples, score, all_ans):
    ...
    return score
def split_by_global(data):

    snapshot = np.array(data[:,:3]).copy()
...
def build_sub_graph(num_nodes, num_rels, triples, use_cuda, gpu):
    ...
    :param src_no_r: relabeled src_no_r id
    ...
    g.apply_edges(lambda edges: {'norm': edges.dst['norm'] * edges.src['norm']})#the norm of edge
    g.edata['type'] = torch.LongTensor(rel)#图上每个关系的类型
...
def build_sub_graph_test(num_nodes, num_rels, triples, use_cuda, gpu):
    """
    :param node_id: node id in the large graph
    :param num_rels: number of relation
    :param src_no_r: relabeled src_no_r id
...
def build_sub_graph_time(num_nodes, num_rels, triples, use_cuda, gpu):
    """
    :param node_id: node id in the large graph
    :param num_rels: number of relation
    :param src_no_r: relabeled src_no_r id
...
def build_empty_graph(num_nodes, num_rels, use_cuda, gpu):
    """
    :param node_id: node id in the large graph
    :param num_rels: number of relation
    :param src_no_r: relabeled src_no_r id
...
def stat_ranks(rank_list, method):
    ...
    logging.info("MRR ({}): {:.6f}".format(method, mrr.item()))
    for hit in hits:
        avg_count = torch.mean((total_rank <= hit).float())
        logging.info("Hits ({}) @ {}: {:.6f}".format(method, hit, avg_count.item()))
    return mrr
...
def load_all_answers_for_filter(total_data, num_rel, rel_p=False):
    ...
    for line in total_data:
        s, r, o = line[: 3]
        if rel_p:
            add_relation(s, o, r, all_ans)#{[s][o]:{r}}
            add_relation(o, s, r + num_rel, all_ans)#{[o][s]:{r}}
        else:
            add_subject(s, o, r, all_ans, num_rel=num_rel)#{o:{r:set()}}
            add_object(s, o, r, all_ans, num_rel=0)#{s:{r:set()}}
    return all_ans
...
def split_by_time(data):
    ...
    logging.info("# Sanity Check:  ave node num : {:04f}, ave rel num : {:04f}, snapshots num: {:04d}, max edges num: {:04d}, min edges num: {:04d}, max union rate: {:.4f}, min union rate: {:.4f}"
...
def slide_list(snapshots, k=1):
    ...
    if k > len(snapshots):
        logging.info("ERROR: history length exceed the length of snapshot: {}>{}".format(k, len(snapshots)))
    for _ in tqdm(range(len(snapshots)-k+1)):
        yield snapshots[_: _+k]

def load_data_adv(dataset, bfs_level=3, relabel=False):
    ...
    elif dataset in ['ICEWS18', 'ICEWS14_no_r', "GDELT", "SMALL", "ICEWS14s", "ICEWS05-15","YAGO",
                     "WIKI"]:
        return knwlgrh.load_from_local_adv("../data", dataset)
    else:
...
def get_dgl_from_list(snap_dlg,snap_history):
    snap_history_dgl = list()
    for time_idx, snap in enumerate(snap_history): #snap:{0:[1,2] 1:[2,4]}
        entity_hist_dgl = {}
        for key in snap.keys():
            entity_hist_dgl[key] = [snap_dlg[idx] for idx in snap[key]]
        snap_history_dgl.append(entity_hist_dgl.copy())
    return snap_history_dgl

def get_dgl_from_sort_list(snap_dlg,snap_list):
    #snap:{0:[1,2] 1:[2,4]}
    entity_hist_dgl = []
    for key in snap_list:
        entity_hist_dgl.append(snap_dlg[key])
    return entity_hist_dgl


def list_merge(mlist):
    length = [len(i) for i in mlist]
    merge_list = list()
    for i in mlist:
        merge_list = merge_list + i
    return merge_list, length

def unbatch(merge_list,length):
    unbatch = list()
    index = 0
    for i in length:
        if index == index + i:
            ...
            unbatch.append(merge_list[index:index + i])
            index = index + i
        return unbatch
...
def load_rule_file(data_dir,status):
    if status == "train":
        data_dir = "../data/"+data_dir+"/train"
    elif status == "valid":
        data_dir =  "../data/"+data_dir+"/valid"
    else:
        data_dir = "../data/"+data_dir+"/test"

    file_name = [(i.split(".")[0]) for i in os.listdir(data_dir)]

    return file_name

def load_json(data_dir):
    data_dir="../data/"+data_dir+"/ts2id.json"
    return json.load(open(data_dir))


def load_init(data_dir,file_path):
    print(os.getcwd())
    data_path = os.path.join(os.path.join("../data",data_dir),file_path)
    with open(data_path, 'rb') as f:
        data = pickle.load(f)
    return data

def init_embedding(data):
    logging.info("==================================loading_init_entity========================================================")
    all_tensor = None
    for d in data:
        if all_tensor is None:
            all_tensor = d['emb'].reshape(1, -1)
            continue
        all_tensor = torch.cat((all_tensor, d['emb'].reshape(1, -1)), dim=0)
    return all_tensor

def init_relation_embedding(relation_tensor,data):
    logging.info("==================================loading_init_relation========================================================")
    num_relation = int(relation_tensor.size()[0]/2)
    print()
    for idx,d in enumerate(data):
        relation_tensor[idx] = d['emb'].reshape(1, -1)
        relation_tensor[idx+num_relation] = d['emb_inv'].reshape(1,-1)
